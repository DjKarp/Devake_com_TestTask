...
public class FP_Controller : MonoBehaviour
{
	...
	void Update()
	{
		...
		if (GameManager.Instance.GetPlayerPawn() != null)
		{
			...
			if ((inputX != 0 | inputZ != 0) & !run) GameManager.Instance.GetPlayerPawn().SetAnimationState(Pawn.PawnAnimatorState.Walk);
			else if (run) GameManager.Instance.GetPlayerPawn().SetAnimationState(Pawn.PawnAnimatorState.Run);
			else GameManager.Instance.GetPlayerPawn().SetAnimationState(Pawn.PawnAnimatorState.Idle);
		...
		if (jumpState == 0 && CanStand() && jump && jumpTimer >= antiBunnyHopFactor)
		{
			...
			if (SoundAndMusic.Instance != null) SoundAndMusic.Instance.PlayPlayerJump(gameObject);
	...
	void OnControllerColliderHit (ControllerColliderHit hit) 
	{
		...
		if (!IsGrounded() && landTimer == 1) if (SoundAndMusic.Instance != null) SoundAndMusic.Instance.PlayPlayerJump(gameObject);
	...
	public FP_Input GetPlayerInput()
	{
